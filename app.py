...
try:
    ...
    # 2. إعداد الجوجل شيت مع معالجة الـ private_key لعدم حدوث خطأ PEM
    scope = ["https://www.googleapis.com/auth/spreadsheets"]
    gcp_json = json.loads(st.secrets["GCP_SERVICE_ACCOUNT"])
    
    # تصليح علامات السطور الجديدة في المفتاح السري أوتوماتيكياً
    ...
    st.sidebar.success("🟢 متصل بالجوجل شيت والذكاء الاصطناعي بنجاح!")
    ...
    if not branch:
        ...
        with st.spinner("جاري تشغيل الذكاء الاصطناعي ومراجعة الـ 8 مستندات..."):
            try:
                ...
                # تخزين البيانات في الـ Session State لعرضها وتأكيدها
...
# عرض البيانات المكتشفة للتأكيد والحفظ
if "extracted_data" in st.session_state:
    ...
    with col1:
        final_branch = st.text_input("الفرع", value=st.session_state["current_branch"])
    ...
    if st.button("💾 حفظ البيانات وإرسالها للصراف"):
        try:
            ...
            st.error(f"خطأ أثناء الحفظ في الشيت: {sheet_err}")
